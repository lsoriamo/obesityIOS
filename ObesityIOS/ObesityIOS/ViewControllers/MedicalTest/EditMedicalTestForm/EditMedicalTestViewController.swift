//
//  EditMedicalTestViewController.swift
//  ObesityIOS
//
...
class EditMedicalTestViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        ref.child("users/\(userId!)/medical_tests/cites/\(timestamp!)").observeSingleEvent(of: .value) { (snapshot) in
            
            let value = snapshot.value as? NSDictionary
            
            var timestampCite:Int64 = value?["timestampCite"] as? Int64 ?? -1
            let doctorCite:String = value?["doctorCite"] as? String ?? ""
            let prescriberName:String = value?["prescriberName"] as? String ?? ""
            let placeCite:String = value?["placeCite"] as? String ?? ""
            let name:String = value?["name"] as? String ?? ""
//            let innerType:Int = value?["innerType"] as? Int ?? -1
            let description = value?["description"] as? String ?? ""
            let prescriberComment = value?["prescriberComment"] as? String ?? ""
            
            if doctorCite != "" {
                self.tfPersonaPrueba.text = doctorCite
                
            }
            
            if placeCite != "" {
                self.tfLugarCita.text = placeCite
                
            }
            
            if prescriberComment != "" {
                self.tfComentarioMedico.text = prescriberComment
            }
            
            if prescriberName != "" {
                self.tfMedico.text = prescriberName
                
            }
            
            if description != "" {
                self.tfDescripcionPrueba.text = description
                
            }
            
            if name != "" {
                self.tfTipoPrueba.text = name
                
            }
            
            if timestampCite != -1 {
                
                timestampCite = timestampCite/1000
                
                let citeDate:Date = Date(timeIntervalSince1970: TimeInterval(timestampCite))
                
                
                self.dpFechaCita.date = citeDate
    ...
    @IBAction func actionBtnEditMedicalTest(_ sender: Any) {
        ...
        if tipoPrueba.isEmpty {
            ...
            let urlMedicalTest = "users/\(userId!)/medical_tests/cites/\(timestamp!)"
...
//            self.ref.child("\(urlMedicalTest)/timestamp").setValue(timestampActual)
            ...
            self.performSegue(withIdentifier: "toMedicalTestBackFromEditSegue", sender: nil)
